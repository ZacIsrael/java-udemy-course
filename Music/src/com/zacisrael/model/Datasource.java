...
public class Datasource {
    ...
    public static final String CONNECTION_STRING = "jdbc:sqlite:/Users/zacisrael/Desktop/UdemyJava/Music/" + DB_NAME;
    ...
    public static final String CLUMN_SONG_ID = "_id";
    public static final String COLUMN_SONG_TRACK = "track";
    public static final String COLUMN_SONG_TITLE = "title";
    public static final String COLUMN_SONG_ALBUM = "album";
    public static final int INDEXX_SONG_ID = 1;
    public static final int INDEX_SONG_TRACK = 2;
    public static final int INDEX_SONG_TITLE = 3;
    public static final int INDEXX_SONG_ALBUM = 4;
...
//            COLUMN_SONG_TITLE + " = ?";
    ...
    public static final String QUERY_VIEW_SONG_INFO_PREP = "SELECT * FROM " + TABLE_ARTIST_SONG_VIEW + " WHERE " +
            COLUMN_SONG_TITLE + " = ?";
...
//            }
            ...
            // return generated key = primary key in the table, automatically generated, id
    ...
    public List<Artist> queryArtists(int sortOrder) {
        ...
        // Try with resources will close both the Statement and the ResultSet automatically so there
        // is no need for a finally block to close them
        try (Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery(sb.toString())) {

            List<Artist> artists = new ArrayList<>();
            while (results.next()) {
                // Creates a list containing all of the artists in the Artist table
    ...
    public List<String> queryAlbumsForArtist(String artistName, int sortOrder) {
        ...
        order(sortOrder, sb, QUERY_ALBUMS_BY_ARTIST_SORT);
    ...
    public List<SongArtist> queryArtistsForSong(String songName, int sortOrder) {
        ...
        sb.append(songName);
        sb.append("\"");
        order(sortOrder, sb, QUERY_ARTIST_FOR_SONG_SORT);
    ...
    private void order(int sortOrder, StringBuilder sb, String queryArtistForSongSort) {
        if (sortOrder != ORDER_BY_NONE) {
            sb.append(queryArtistForSongSort);
            if (sortOrder == ORDER_BY_DESC) {
                sb.append("DESC");
            } else {
                sb.append("ASC");
    ...
    public int getCount(String table) {
        ...
        try (Statement statement = conn.createStatement();
            ...
            // the count will be in "count" column
            return count;
    ...
    public List<SongArtist> querySongInfoView(String title) {
        try {
            ...
            resultSetLoop(results, songArtists);
            return songArtists;
    ...
    private int insertArtist(String name) throws SQLException {
        ...
        // querying the artist table to see if an artist with the name parameter as their name already exists.
        ...
        if (results.next()) {
            ...
            // artist is not in  the table, so we must insert it into the table
            insertIntoArtists.setString(1, name);
            int affectedRows = insertIntoArtists.executeUpdate();
            // should be one added row to the table
            if (affectedRows != 1) {
                // There is an issue
    ...
    private int insertAlbum(String name, int artistId) throws SQLException {
        ...
        if (results.next()) {
            // arlbum is already in the table
            return results.getInt(1);
            // return the artist's id
        } else {
            // artist is not in  the table, so we must insert it into the table
            insertIntoAlbums.setString(1, name);
            // first column in album table is the album name
            insertIntoAlbums.setInt(2, artistId);
            // second column in album table is the artist's id
    ...
    public void insertSong(String title, String artist, String album, int track) {
        ...
        try {
            ...
            // starting a transaction by turning off the autocommit behavior of the connection

            // insert the artist & the album
            ...
            if (affectedRows == 1) {
                conn.commit();
                // done, so we commit the changes which ends the transaction
            } else {
            ...
            try {
                ...
                conn.rollback(); // backs out any transactions made since the transaction was started
            ...
            try {
                ...
                // return to the default autocommit behavior
            } catch (SQLException e) {
    ...
    private void resultSetLoop(ResultSet results, List<SongArtist> songArtists) throws SQLException {
        while (results.next()) {
    ...
    private List<SongArtist> getList(StringBuilder sb) {
        try (Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery(sb.toString())) {
...
/*
        ...
        A PreparedStatement represents a precompiled SQL statement that can be executed multiple times
        without having to recompile for every execution.

        What is a transaction in sql?
        A transaction is a logical unit of work that contains one or more SQL statements.
         A transaction is an atomic unit. The effects of all the SQL statements in a transaction can be either
         all committed (applied to the database) or all rolled back (undone from the database).

         We use transactions when we want to perform a series of sql statements as a unit
         so that the changes are only committed to the database if all the statements ran successfully.
     */
